...
static BST *read_dictionary(char *filename)
{
    ...
    while(!feof(dictfile)) {
        fgets(wordbuf, BUFSIZE, dictfile);		//get word from dictionary
		int i;        
		for(i = 0; wordbuf[i] && wordbuf[i] != '\n'; i++); // do nothing loop (except increment i until reaching end of word)
		wordbuf[i] = '\0';						
        char * key = strdup(wordbuf);			///*???*/// Why duplicate?: Can't copy word onto new pointer directly?
        dict = bst_insert(dict, key, NULL);		//////////////////////////////
        ///*???*/// key is whole one word
    }
...
char *get_next_word(FILE *inf)
{
    ...
    while((c = fgetc(inf)) != EOF) {	//skip non-alphabet && newline
        if( c == '\n') {
            linecount++;				///*???*///
        }
        if (isalpha(c)) {
    ...
    if(feof(inf)) {					///*???*/// This should be in while loop?
        return NULL;
    }
        ...
        while( (c = fgetc(inf)) != EOF) {
            if(isalpha(c)) {
                ...
                ungetc(c, inf);		//break if there is non-alphab amongst alphab
                break;
            }
        }

        wordbuf[buf_ind] = '\0';
        return strdup(wordbuf);   	///*???*/// Why duplicate?
     }
}
