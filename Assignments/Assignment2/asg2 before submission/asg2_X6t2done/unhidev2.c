 ...
 * 
 * LIMITATION: Will output file open error if embeded message does not end with
 * 		'\0'.
...
int readMsgfromPPMs1 (char buffer, FILE *fin, int *countBit, char *letter) {	
	while(!(feof(fin))){
		...
		if(*letter=='\0' && (*countBit)<0){		// actually '\0'
			return -1;
		}
		if (feof(fin)){		//if may continue with another file
			// come here after the sibling while loop above terminates.
			return 4;
		}
		else {
			printf("%c", *letter);	
