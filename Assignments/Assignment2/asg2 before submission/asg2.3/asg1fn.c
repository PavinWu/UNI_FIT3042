...
int awMan(int awManCase){
	char *errorDesc[] = {			 //awManCase:
		"Input error\n",			 //1
		"Invalid file Name\n",
		"Cannot allocate memory for message/variable\n",
		"File open error\n",
		"PPM magic number not P6\n", //5
		"Width not numbers\n",
		"Height not numbers\n",
		"Color should be numeric of value between 1 and 65536\n",
		"Header incomplete\n",		 //9
		"\nPPM header and actual size unmatched. Truncated file detected.\n", 
		"Non-single -m, -p or -s flag detected. Please try again.\n", //11
		"Number of ppm files must be integer between 1 and 255\n"
	};
	printf("%s", errorDesc[awManCase-1]);
	return awManCase;
} 
...
int headerChecking(FILE *fp, int *width, int *height, int *color, char *buffOut){
	...
	return 9;
	/* Gets here only when file terminates prematurely, i.e. header is incomplete*/
}
