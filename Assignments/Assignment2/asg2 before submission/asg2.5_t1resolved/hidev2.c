...
int main(int argc, char *argv[]) {
	...
	if (lastEmbedBin==NULL || counter==NULL || totSize==NULL) {return awMan(3);}
	...
	*counter = 0; //Counter is current message written on file
	...
	if (opmode==1){
		...
		if (finname == NULL) {return 3;}
		foutname = (char *) malloc(sizeof(char)*(strlen(argv[4])+9));
		if (foutname == NULL) {return 3;}
	...
	if (finname==NULL || foutname==NULL) {return awMan(3);}
	/***************************/
