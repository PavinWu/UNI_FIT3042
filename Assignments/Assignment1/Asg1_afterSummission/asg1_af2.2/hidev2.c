 ...
 * 
 * ip: pointer to i counter from main
 * msgCount: size of message (not include '\0') to encode
 * 
 * ip and msgCount are for 
 */
int embedBinary(char *buffer, int binary[], FILE *fin, FILE *fout){
	...
	for (j=0; j<8; j++){	 		
		...
		else {		//ppm actually not as large as specified in header
			return 10;
		}
	}
	return 0;
...
int writeMsg(char buffer, int width, int height, FILE *fin, FILE *fout) {
	// Take & write message	
	int maxChar_int = (int) (width*height)*3/8;			
	...
	*(msg+msgCount) = '\0';	//add string terminating character
	...
	int i;
	int err;
	for (i=0; i<msgCount+1; i++){	//for each character from msg (include '\0')
		toBinary(msg[i], binary, 8);	 //get 8 bit binary of single character
		err = embedBinary(&buffer, binary, fin, fout);
		if (err) {return err;}
	}
	if (fread(msg, 1, 1, stdin) != 0){	//if still msg after ppm run out of space 
